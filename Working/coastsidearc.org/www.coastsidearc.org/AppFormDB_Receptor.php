...
if (!$conn) {
    ...
    $fullname = quote($jsonData['fullname']);
    ...
    $pp_total = $jsonData['pp_total'];
    $pay_paypal = quote($jsonData['pay_paypal']);
    $new = quote($jsonData['newmember']);
    $transaction_status = quote($jsonData['transaction_status']);
    ...
    if ($new_id <> null) {
        $q_id = quote($new_id);
        $sql = "INSERT INTO pp_tnx ( `pp_id`, `mydate`, `FullName`, `years`, `callsigns`, `primary`, `family`, " .
        " `repeater`, `digipeater`,`subtotal`, `paypalfee`, `clubreceives`, `total`, `pp_total`, `pay_paypal`, `new`, `transaction_status` ) VALUES ( " .
        " $q_id, $date, $fullname, $years, $callsigns, $primary, $family, $repeater, $digipeater, " .
        " $subtotal, $paypalfee, $clubreceives, $total, $pp_total, $pay_paypal, $new, $transaction_status)";
