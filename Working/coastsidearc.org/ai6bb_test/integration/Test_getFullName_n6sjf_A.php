...
<head>
    ...
    <script>
        ...
        const Expected = ['Paul Atkins', 'Jillian Aldersen', 'Michael S. Herbert',
            'Frank C. Erbacher', 'William J. Anderson', 'Fernel Andong',
            'Steve Austin', 'Bharat Bailur', 'Robert Barbitta', 'Tom Oliver',
            'Gary Barnes', defaultName, defaultName, 'Jonathan Lancelle'];

        function isAtPos(arr, obj) {
            for (var ix = 0; ix < arr.length; ix++) {
                if (arr[ix] == obj) return ix;
            }
            return (-1);
        }
                    ...
                    if ((this.status === 200) && (this.readyState === 4)) {
                        ...
                        if (response == '\r\n') {
                            ...
                            if (pdata.result === null) {
                                myTestCallback( pdata.callsign, defaultName);
                            } else {
        ...
        function myTestCallback( callsign, actualName ) {
            ...
            var ix = isAtPos(callsigns, callsign);
            var expected = (ix >= 0 ? Expected[ix] : defaultName);
            ...
            test = (expected === actualName ? "Passed" : "Fail");
